...
class InstanceDetector:
    ...
    def __init__(self, window_seconds=5):
        ...
        self.non_compliance_delay = 0  # seconds before declaring non-compliant(set to zero to test with the still images)
    ...
    def process_detection(self, all_detections, dev_mode=False, settings=None):
        ...
        has_negative_ppe = any(
            cls.startswith("no-")
            for cls in detected_classes
        )

        if person_count > 0 and not has_positive_ppe and not has_negative_ppe:
